...
namespace Tac
{
  namespace Asteroids
  {
    ...
    void Gameplay::Update( float dt )
    {
      ...
      // copy game state to gAsteroids vector
      gAsteroidsObjects.clear();
      for (std::pair<Object*const, SPObject> & pair: mAsteroids.mObjects)
      {
        Object * obj = pair.first;
        gAsteroidsObjects.push_back(*obj);
      }
    ...
    void Gameplay::SendGamestate()
    {
      ...
      list->mNumObjs = gAsteroidsObjects.size();
      if (!gAsteroidsObjects.empty())
      {
        int numToSend = std::min((int)gAsteroidsObjects.size(), MAXOBJS);
        memcpy(
          list->mObjs,
          &gAsteroidsObjects[0],
          sizeof(Object) * numToSend);
      }
