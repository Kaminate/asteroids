...
namespace Tac
{
  ...
  Matrix4 Camera::GetPerspectiveProjMatrix()
  {
    //                                       /----- [x,y,z]
    //                          [x'y'z']/----
    //                          /----|
  ...
  Tac::Matrix4 Camera::GetInverseViewMatrix()
  {
    Vector3 camR;
    Vector3 camU;
    Vector3 negZ = -mViewDirection;
  ...
  Tac::Matrix4 Camera::GetOrthographicProjMatrix()
  {
    float a = -2/(mFar - mNear);
    float b = -(mNear + mFar) / (mFar - mNear);

    // aligns -near to 0, -far to 1
    return Matrix4(
      2/mWidth, 0, 0, 0,
      0, 2/mHeight, 0, 0,
      0, 0, a, b,
