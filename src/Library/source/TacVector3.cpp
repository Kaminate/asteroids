...
namespace Tac
{
  ...
  TacDLLAPI void Vector3::Min( const Vector3& rhs )
  {
    x = std::min(x, rhs.x);
    y = std::min(y, rhs.y);
    z = std::min(z, rhs.z);
  ...
  TacDLLAPI void Vector3::Max( const Vector3& rhs )
  {
    x = std::max(x, rhs.x);
    y = std::max(y, rhs.y);
    z = std::max(z, rhs.z);
  ...
  TacDLLAPI float Vector3::Dist( const Vector3 & rhs ) const
  {
    float dx = x - rhs.x;
    float dy = y - rhs.y;
    float dz = z - rhs.z;
  ...
  TacDLLAPI float Vector3::DistSq( const Vector3 & rhs ) const
  {
    float dx = x - rhs.x;
    float dy = y - rhs.y;
    float dz = z - rhs.z;
